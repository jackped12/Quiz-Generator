...
await test('diagram schemas validate kinds and limits while allowing older packs',()=>{
 for(const type of ['flow','comparison','concept-map']){const p=structuredClone(fixture);p.lessons[0].diagrams[0].type=type;studySchema.parse(p);}
 const legacy=structuredClone(fixture);delete legacy.lessons[0].diagrams;studySchema.parse(legacy);
 for(const change of [d=>d.type='script',d=>d.items=[],d=>d.items=Array(7).fill(d.items[0]),d=>d.items[0].label='']){const p=structuredClone(fixture);change(p.lessons[0].diagrams[0]);assert.throws(()=>studySchema.parse(p));}
});
await test('render all diagram types, escape source text, and show legacy concepts',async()=>{
 await build({stdin:{contents:"export {GuideDiagrams} from './portable/diagram';export {renderToStaticMarkup} from 'react-dom/server';export {createElement} from 'react';",resolveDir:process.cwd(),loader:'tsx'},outfile:path.join(scratch,'diagrams.cjs'),bundle:true,platform:'node',format:'cjs',jsx:'automatic'});
 const {GuideDiagrams,renderToStaticMarkup,createElement}=await import('file:///'+path.join(scratch,'diagrams.cjs').replaceAll('\\','/'));
 for(const type of ['flow','comparison','concept-map']){const lesson=structuredClone(fixture.lessons[0]);lesson.diagrams[0].type=type;lesson.diagrams[0].items[0].label='<script>alert(1)</script>';const html=renderToStaticMarkup(createElement(GuideDiagrams,{lesson}));assert(html.includes('diagram-'+type));assert(html.includes('&lt;script&gt;'));assert(!html.includes('<script>'));assert(html.includes('Build sugars'));}
 const lesson=structuredClone(fixture.lessons[0]);delete lesson.diagrams;const html=renderToStaticMarkup(createElement(GuideDiagrams,{lesson}));assert(html.includes('CONCEPT MAP'));assert(html.includes('Chlorophyll'));
});
